...
export default {
  ...
  s1: { name: 'Drawdown', kind: 'lob', air: true, dmg: 8, kb: 5, kbScale: 4, kbAngle: 50, speed: 4.6, vy: -6.8, grav: 0.34, w: 20, h: 16, shape: 'binder', color: '#27425f', cooldown: 280, zoneOnLand: { type: 'ember', look: 'paper', w: 128, life: 260, ownerImmune: true, burn: 2 }, startup: 13, active: 2, recover: 17 },
  s2: { name: 'Dad Reflexes', kind: 'catch', stance: 30, recover: 20, cooldown: 240 },
  super: { name: 'Enforcement', kind: 'shout', dmg: 12, kb: 8, kbScale: 14, kbAngle: 40, range: 90, startup: 24, active: 6, recover: 26, aiRange: [40, 150] },   // parryable; collects a LIEN for +8,
  ...
  hooks: {
    // LIEN: Seelye's next special on a marked opponent collects +4; Enforcement
    // (the super) collects +8 — mark with Leverage, then collect.
    preHit(f, def, dmg, slot, move) {
      if ((slot === 'special' || slot === 'super') && def.hasStatus('lien')) {
        def.clearStatus('lien');
        const enforced = slot === 'super';
        f.world.fx.text(def.x, def.y - 140, enforced ? 'LIEN ENFORCED!' : 'LIEN COLLECTED!', '#c9a227');
        return dmg + (enforced ? 8 : 4);
      }
      return dmg;
    },
  },
